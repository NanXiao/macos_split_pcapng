...
then
	...
	export ORIG_SPLIT_DIR_NAME=$2
fi

# Iterate interface from pcapng file
for nic in $(capinfos "$1" | grep 'Name =' | awk '{print $3}')
do
	export SPLIT_DIR_NAME="${ORIG_SPLIT_DIR_NAME}/${nic}"

	mkdir -p "${SPLIT_DIR_NAME}"
	tcpdump -k PN -qnr "$1" -Q if="${nic}" | ./split_based_on_pid.awk
